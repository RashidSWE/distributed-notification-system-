...
export const create = async (req: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    if (existingUser) {
      return reply.code(400).send(
        errorResponse('Registration failed', 'User already exists')
      )
    }
  ...
  return reply.send(
    ...
    return reply.code(500).send(
      errorResponse("Failed to register user", err instanceof Error ? err.message : "Unknown error")
    );
  }
};
...
export const login = async (req: any, reply: FastifyReply) => {
  ...
  return reply.send(
    ...
    return reply.code(401).send(
      errorResponse("Login failed",err instanceof Error ? err.message : "Unknown error")
    );
  }
};
...
export const getUserProfile = async (req: any, reply: FastifyReply) => {
  try {
    ...
    return reply.code(500).send(
      errorResponse("Failed to retrieve profile",err instanceof Error ? err.message : "Unknown error")
    );
  }
}
...
export const updateUserPreferences = async (req: any, reply: FastifyReply) => {
  try {
    ...
    return reply.code(500).send(
      errorResponse("Failed to update preferences",err instanceof Error ? err.message : "Unknown error")
    );
  }
};

export const handlePushTokenUpdate = async (req: any, reply: FastifyReply) => {
  try {
